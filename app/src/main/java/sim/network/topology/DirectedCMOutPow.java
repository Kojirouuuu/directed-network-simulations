 ...
 */
public final class DirectedCMOutPow {
     ...
     * @return 生成された DirectedGraph
     */
    public static DirectedGraph generate(String name, int n, int kOutMin, int kOutMax, double kuAve, double gamma, long seed) {
        validateGenerateParams(name, n, kOutMin, kuAve, gamma);

        Random random = new Random(seed);

        int[] ko = sampleOutDegreesPowerLaw(n, kOutMin, kOutMax, gamma, random);
        int[] ki = assignInDegreesRandomly(n, (int) sum(ko), random);
    ...
    private static void validateGenerateParams(String name, int n, int kOutMin, double kuAve, double gamma) {
        if (name == null) throw new IllegalArgumentException("name must be non-null");
        if (n < 0) throw new IllegalArgumentException("n must be non-negative");
        if (kOutMin < 0) throw new IllegalArgumentException("kOutMin must be non-negative");
    ...
    /**
     * パワーロー分布に従う出次数列をサンプルする。
     */
    private static int[] sampleOutDegreesPowerLaw(int n, int kOutMin, int kOutMax, double gamma, Random random) {
        double[] cdf = buildPowerLawCdf(kOutMin, kOutMax, gamma);
        int[] ko = new int[n];
        for (int u = 0; u < n; u++) {
            double r = random.nextDouble();
            int idx = Arrays.binarySearch(cdf, r);
            ko[u] = idx >= 0 ? idx + kOutMin : -idx - 1 + kOutMin;
        }
        return ko;
    }

    private static double[] buildPowerLawCdf(int kOutMin, int kOutMax, double gamma) {
        int len = kOutMax - kOutMin + 1;
        double[] cdf = new double[len];
        double s = 0.0;
        for (int k = kOutMin; k <= kOutMax; k++) {
            s += Math.pow(k, -gamma);
            cdf[k - kOutMin] = s;
        }
        for (int i = 0; i < len; i++) {
    ...
    /**
     * 総入次数 totalIn を n ノードにランダムに配分する。
     */
    private static int[] assignInDegreesRandomly(int n, int totalIn, Random random) {
        int[] ki = new int[n];
