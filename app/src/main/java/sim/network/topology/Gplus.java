...
/**
 * Google+ (gplus) のエッジリストから有向グラフを読み込む。
 ...
 * の gplus_combined.txt を使用する。
 *
 * @see EgoTwitter#loadFromEdgeList(String, Path)
 */
public final class Gplus {

    private static final String DEFAULT_PATH1 = "app/data/real-network/gplus_combined.txt";
    private static final String DEFAULT_PATH2 = "data/real-network/gplus_combined.txt";
    ...
    /**
     * デフォルトのエッジリストを読み込む。
     * まず {@code app/data/real-network/gplus_combined.txt} を探し、
     * 見つからなければ {@code data/real-network/gplus_combined.txt} を試す。
     *
     * @return 構築された DirectedGraph（名前は "gplus"）
    ...
    public static DirectedGraph loadFromDefaultEdgeList() throws IOException {
        ...
        return EgoTwitter.loadFromEdgeList("gplus", p);
    }
}
