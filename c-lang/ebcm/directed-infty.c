...
DegreeDist *build_all_degree_dist(const EBCMConfig *cfg) {
    ...
    /* mean を先に設定（get_effective_degree_max と create_degree_dist で使う） */
    D->mean_ki = (strcmp(ki->type, "Pow") == 0)
                     ? calculate_mean_powerlaw(ki->min, ki->max, ki->gamma)
                     : ki->mean;

    D->ki_min = (strcmp(ki->type, "Pow") == 0) ? ki->min : 0;
    /* ループ範囲は実効 max */
    D->ki_max = get_effective_degree_max(ki->type, D->mean_ki, ki->max);

    // 配列生成（Poisson のとき min は無視される設計）
    D->Pi = create_degree_dist(D->mean_ki, ki->min, ki->max, ki->gamma, ki->type);
...
static double Theta_d_prime(const double *Binom, int ki, int kmax, int T, double theta_d) {
    ...
    if (T == 1) {
        ...
        return (double)ki * pow(theta_d, (double)(ki - 1));
    }
    ...
    sum += Binom[ki * (kmax + 1)] * (ki * pow(theta_d, (double)(ki - 1)));
    for (int md = 1; md <= md_max; md++) {
        sum += Binom[ki * (kmax + 1) + md] *
               ((ki - md) * pow(q, (double)md) * pow(theta_d, (double)(ki - md - 1)) -
                md * pow(q, (double)(md - 1)) * pow(theta_d, (double)(ki - md)));
    }
    return sum;
}
...
/* dR/dt = μ * A,  A = 1 - S - R（活動中＝感染中） */
static double rhs_R(const DegreeDist *D, const DynamicsConfig *p, const double *Binom,
                    double theta_d, double R) {
    double S = compute_S(D, p, Binom, theta_d);
    double A = 1.0 - S - R;
    if (A < 0.0)
        A = 0.0;
    return p->mu * A;
...
#pragma omp parallel for schedule(dynamic)
    for (int task = 0; task < total_tasks; task++) {
        ...
        int n_roots = find_roots(g_d, D, &dynamics, Binom, 0.0, 1.0, theta_search_step, 1e-12, 1000,
                                 roots, MAX_GD_ROOTS);
        ...
        double R_inf = rhs_R(D, &dynamics, Binom, valid_theta_d, 0.0);
        double Phi_inf = rhs_Phi(D, &dynamics, Binom, valid_theta_d);

        double roots_prime[MAX_GD_ROOTS];
        int n_roots_prime = find_roots(g_d_prime, D, &dynamics, Binom, 0.0, 1.0, theta_search_step,
        ...
        for (int r = 0; r < n_roots_prime; r++) {
            ...
            if (root <= 1.0 && root >= 0.0 && delta < delta_min) {
                prime_found = true;
                delta_min = delta;
            }
        }
